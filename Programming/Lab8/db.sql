...
create table if not exists coordinates(id serial primary key, x bigint check (x <= 300) not null, y double precision not null);
...
select into coordinates_id coordinates.id from coordinates join route on route.coordinates_id = coordinates.id where route.id = route_ask_id;
select into location_from_id location_from.id from location_from join route on route.coordinates_id = location_from.id where route.id = route_ask_id;
select into location_to_id location_to.id from location_to join route on route.coordinates_id = location_to.id where route.id = route_ask_id;
...
-- Триггер на удаление всех зависимых рутов
create or replace function delete_dependents() returns trigger as $$

begin

delete from route where route.id in (select route.id from route join collection on collection.route_id = route.id where old.id = collection.user_id);

return old;
end;
$$ language plpgsql;

create trigger delete_dependents_routes before delete on users for each row execute function delete_dependents();